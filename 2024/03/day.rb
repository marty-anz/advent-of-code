...
def part1(input)
  input.join.scan(/mul\(\d+,\d+\)/).sum { |m| m.scan(/\d+/).map(&:to_i).inject(:*) }
end
...
def part2(input)
  input.join.
    ...
    each_with_object([true, []]) { |m, state| m.start_with?('do') ? state[0] = m == 'do()' : (state[1] << m if state[0]) }[1].
    sum {|m| m.scan(/\d+/).map(&:to_i).inject(:*) }
end
